...
2. **Interpret the output** against [`docs/git-workflow.md`](../../../docs/git-workflow.md):
   ...
   - `CANDIDATE` branches are a heuristic, not proof — a branch cut straight off `dev` (escape hatch, pre-build discovery) looks identical to an orphaned ticket branch once `dev` has moved past where it was cut. Check each candidate against Linear before calling it a problem: if its ticket is Done and it long predates the current project branch, it's very likely a leftover that should be deleted; if the ticket's still open or it's genuinely dev-based work, it's not a finding.

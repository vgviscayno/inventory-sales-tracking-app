...
function ProductForm({ product }: { product: Product }) {
  ...
  const [quantityOnHand, setQuantityOnHand] = useState(String(product.quantityOnHand));
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`flex-1 rounded-xl border py-2.5 font-semibold ${
            confirmingDelete ? "bg-danger border-danger text-white" : "text-danger border-line"
          }`}
        >
          {deleting ? "Deleting..." : confirmingDelete ? "Confirm Delete" : "Delete Product"}
        </button>
      </div>
    </main>
  );

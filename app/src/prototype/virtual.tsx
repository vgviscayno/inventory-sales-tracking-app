...
// PROTOTYPE ONLY — throwaway. Shared by the movement-history variants:
// INV-18 (Movement history surface) — https://linear.app/inventory-sales-tracking/issue/INV-18
//
// A windowed, day-grouped list. Headings and rows have different fixed heights,
// so positions come from a prefix-sum offset table rather than a single row
// height. The current day's heading pins as an overlay — reserving no space, and
// only once its own inline heading has scrolled away, so it never reads double.

import { type ReactNode, useMemo, useState } from "react";
import { dayKey, deltaColor, formatDayHeading, signed } from "./ledger";

export type DayListItem<T> =
  | { kind: "header"; key: string; at: number; net: number }
  | { kind: "row"; key: string; row: T };

type Groupable = { _id: string; createdAt: number; quantity: number };

/** Flattens rows (already newest-first) into a heading/row stream. */
export function buildDayList<T extends Groupable>(
  rows: T[],
  headerH: number,
  rowH: number,
) {
  ...
  for (const row of rows) {
    ...
    if (header.kind === "header") header.net += row.quantity;
...
export function WindowedDayList<T extends Groupable>({
  ...
  return (
    <div className="card relative overflow-hidden">
      ...
      {/* PROTOTYPE: surface the windowing so it's visible that it's working */}
      <div className="text-sub border-t border-line px-3 py-1 text-[11px]">
        windowed — {visible.length} of {items.length} items in the DOM (
